...
## 2026-09-07 — Bigger re-runs (n=2000 image, n=400 audio) flip one of the paper's own findings: the image streams' gap is now significant

The 2026-09-02 entry below reported the spatial-vs-frequency AUC gap on images as **not** statistically significant at n=500 (250 final split) — the paired bootstrap's 95% CI included zero (p=0.278). Re-running at 4x the sample size (n=2000 calibration, 1000/1000 validation/final split, `reports/2026-09-07.md`) changes that conclusion: spatial (0.6641 AUC) beats frequency (0.5752 AUC) by 0.0889, with a 95% CI of [0.0397, 0.1365] that now excludes zero (**p=0.0000**). The weight-stability bootstrap agrees with the flip — spatial's weight ranges p10-p90 0.556-0.686 and frequency's 0.314-0.444 no longer overlap, versus the substantially overlapping ranges (0.42-0.78 / 0.22-0.58) reported at n=500. The earlier "not significant" verdict was not wrong given the data available at the time — it was a correct read of an underpowered sample. This is a textbook illustration of why the paired significance test exists at all: a real, if modest, effect can be invisible at n=250 and clearly visible at n=1000, and reporting "not significant" should always have been read as "not enough data to tell," never as "no real difference" (the 2026-09-02 entry said as much, and this is the reason why). Corrected fusion weights on this run are 62.4%/37.6% (spatial/frequency) — a real, measured split, not the near-even 51.5/48.5 the smaller run reported. `packages/core/src/calibration.json` updated accordingly; note the spatial stream's fitted temperature is now 10.0 (heavily overconfident on the calibration corpus), up from a more modest value at n=500 — worth watching in a future run rather than treated as settled.

Re-running the audio harness the same way (n=400 calibration, 200/200 split, `reports/audio-2026-09-07.md`) kept the same qualitative verdict as before (AASIST beats `audio_frequency` significantly, p≈0.0000) but the exact numbers moved: `audio_frequency`'s cross-dataset AUC came in higher this time (0.7709 vs. the earlier 0.6867), narrowing the gap from 0.29 to 0.2089 AUC and shifting the fusion weight from 78.5/21.5 to 74.3/25.7. The fusion-induced AUC penalty got *larger* at this sample size, not smaller — -0.0240 versus the -0.0064 the smaller n=120 run reported — which is reported as-is; there is no reason to expect a `-0.0064`-style small penalty to hold exactly as sample size changes, and the honest number is whatever the current, larger, held-out split actually measures.

Both re-runs took multiple attempts to complete due to unrelated infrastructure instability, not a bug in the harness itself: background eval processes were repeatedly killed when their hosting session ended (an environment/session-lifecycle issue, not a code defect — per-sample caching meant no scored data was lost across restarts), and network conditions degraded partway through one session (DNS failures, a self-signed-certificate SSL error consistent with a network-level HTTPS-inspecting proxy, and connection resets against the large ASVspoof2021/DF40 archives specifically) before recovering on a later attempt. Neither issue affects the validity of the final numbers above, which came from completed, uninterrupted scoring passes.
