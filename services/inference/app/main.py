...
def health() -> HealthResponse:
    ...
    versions: dict[str, str] = {"face_detector": "YuNet (OpenCV zoo, MIT)"}
...
async def analyze(file: UploadFile = File(...)) -> AnalysisReport:
    ...
    if file.content_type not in _ACCEPTED_IMAGE_TYPES:
        raise HTTPException(
            status_code=415,
            detail=(
                f"unsupported content type {file.content_type!r}; "
                f"expected one of {sorted(_ACCEPTED_IMAGE_TYPES)}"
            ),
        )
