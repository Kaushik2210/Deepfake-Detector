...
6. **Fuse** the two streams (weighted by AUC measured on a held-out validation split of the reporting corpus — see below — the same mechanism every other stream in this project uses) and **generate a spectrogram** (STFT magnitude, dB) as the evidence artifact — audio's equivalent of Stream B's frequency plot or Stream A's Grad-CAM heatmap.

The eval harness (`services/inference/eval/audio_run.py`) reported AASIST-alone cross-dataset AUC of **0.9767** (EER 5.0%) on the final held-out half of ASVspoof2021 after calibrating on ASVspoof2019. Four things worth knowing before trusting either number at face value, all documented in full in `DECISIONS.md`:
...
- **Fusing `audio_frequency` in still makes cross-dataset AUC worse**, not better: 0.9767 → 0.9367. Fusion weights are derived from AUC measured on a held-out validation split of the reporting corpus (disjoint from the final split the numbers above come from) rather than the calibration split — a fix for exactly this stream's earlier, worse divergence (0.962 → 0.933, using a calibration-split-derived weight that overestimated its value from in-distribution AUC 0.907 instead of its true cross-dataset AUC). The weight ships as measured either way, and a reader should not assume this stream is a net improvement just because it exists.
- **The identical validation/cross-dataset divergence pattern was first found in the image pipeline** (Phase 3, `frequency` stream) and confirmed independently here in a second modality — see `DECISIONS.md`, 2026-08-28, for both the original finding and the validation-split fix now applied to both harnesses.
- **A paired significance test confirms `audio_frequency`'s shortfall is real, not noise**: 0.29 AUC below AASIST with a 95% CI of [0.163, 0.434] (p≈0.0000), and a weight-stability bootstrap shows no overlap at all between the two streams' fusion-weight ranges across 500 resamples — a settled gap, unlike the near-tied image streams (see `DECISIONS.md`, 2026-09-02).

See `eval/reports/audio-2026-09-02.md` for the full numbers.
