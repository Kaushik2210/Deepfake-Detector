...
The eval harness (`services/inference/eval/audio_run.py`) reported AASIST-alone cross-dataset AUC of **0.9856** (EER 2.0%) on the final held-out half of ASVspoof2021 after calibrating on ASVspoof2019. Three things worth knowing before trusting either number at face value, all documented in full in `DECISIONS.md`:
...
- **Fusing `audio_frequency` in still makes cross-dataset AUC worse**, not better: 0.9856 → 0.9792. Fusion weights are derived from AUC measured on a held-out validation split of the reporting corpus (disjoint from the final split the numbers above come from) rather than the calibration split — a fix for exactly this stream's earlier, worse divergence (0.962 → 0.933, using a calibration-split-derived weight that overestimated its value from in-distribution AUC 0.907 instead of its true cross-dataset AUC of 0.6528). The gap shrank by more than 4x but did not close; the weight ships as measured either way, and a reader should not assume this stream is a net improvement just because it exists.
